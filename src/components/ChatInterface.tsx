...
const ChatInterface = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Chat Header */}
      <div className="flex items-center justify-between p-2 sm:p-4 border-b border-blue-100 dark:border-slate-600 bg-blue-50 dark:bg-slate-700">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
          <span className="text-xs sm:text-sm text-blue-600 dark:text-blue-300 font-medium">Mindful AI</span>
          {currentMood && (
            <span className="text-[10px] sm:text-xs bg-blue-100 dark:bg-slate-600 text-blue-600 dark:text-blue-300 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">
              Mood: {currentMood}/10
            </span>
          )}
        </div>
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            ...
            className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 text-xs sm:text-sm text-blue-600 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-100 hover:bg-blue-100 dark:hover:bg-slate-600 rounded-lg transition-colors"
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4 bg-white dark:bg-slate-800">
      ...
      {/* Input */}
      <div className="p-2 sm:p-4 border-t border-blue-100 dark:border-slate-600 bg-white dark:bg-slate-800">
        {isLimitReached ? (
          <div className="text-center py-2">
            ...
            <button
              onClick={() => setShowExportDialog(true)}
              className="px-3 sm:px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-xs sm:text-sm"
            >
              Export Conversation
            </button>
          </div>
          ...
          <div className="flex gap-2">
            <input
              ...
              disabled={isLoading}
              className="flex-1 px-3 sm:px-4 py-2 text-sm sm:text-base border border-blue-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50"
            />
            <button
              onClick={handleSendMessage}
              disabled={isLoading || !inputText.trim()}
              className="px-3 sm:px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
            </button>
          </div>
        )}
        <div className="text-[10px] sm:text-xs text-blue-400 dark:text-blue-300 mt-2 text-center">
